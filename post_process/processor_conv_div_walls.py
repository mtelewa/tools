...
class TrajtoGrid:
    ...
    def get_chunks(self, stable_start, stable_end, pump_start, pump_end,
                        beginConvergeX, endConvergeX, beginDivergeX, endDivergeX):
        """
        ...
        beginConvergeX: float, Narrow region begin
        endConvergeX: float, Narrow region end
        beginDivergeX: float, Wide region begin
        endDivergeX: float, Wide region end
        ...
        # avg_fluid_begin_conv, avg_fluid_end_conv = np.mean(comm.allgather(np.mean(fluid_begin_conv))), \
        #                                            np.mean(comm.allgather(np.mean(fluid_end_conv)))
        avg_fluid_begin_div, avg_fluid_end_div = np.mean(comm.allgather(np.mean(fluid_begin_div))), \
                                                 np.mean(comm.allgather(np.mean(fluid_end_div)))

        # Define the upper surface and lower surface regions
        # Start from positive value
        ...
        Lz = utils.extrema(surfU_zcoords_div)['global_max'] - avg_surfL_begin
        cell_lengths_updated = [Lx, Ly, Lz]
        domain_min = [0, 0, avg_surfL_begin]

        # Fluid domain dimensions
        fluid_lengths = [Lx, Ly, (avg_fluid_end_div + avg_surfU_begin_div) / 2. - (avg_fluid_begin_div + avg_surfL_end_div) / 2.]  # [fluid_max[0]- fluid_min[0], fluid_max[1]- fluid_min[1], avg_gap_height_div]   #
        fluid_domain_min = [0, 0, (avg_fluid_begin_div + avg_surfL_end_div) / 2.]
        ...
        # Fluid partition-----------------------------------------
                ...
                # # # # if rank==0:
                # # #     # print(maskx_bulk.shape)
                # a = N_fluid_mask[:,i,k]
                #     # print(N_bulk_mask[60:,i,k])
                # if rank==0:
                #     print(a[0])
        ...
        # -----------------------------------------------------
                ...
                temp_ch_solid[:, i, k] =  np.sum(mass_surfU_vib * sci.gram / sci.N_A * \
                                        ...
                                        ((3 * N_Upper_vib_mask[:, i, k] - 3) * sci.k)
