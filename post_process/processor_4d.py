...
class TrajtoGrid:
    ...
    def __init__(self, data, start, end, Nx, Ny, Nz, mf, A_per_molecule, fluid, tessellate=0, TW_interface=1):
    ...
    def get_chunks(self, stable_start, stable_end, pump_start, pump_end):
        ...
        if solid_start != None:
            ...
            surfU_vib_xcoords, surfU_vib_ycoords, surfU_vib_zcoords = solid_xcoords[:,surfU_vib_indices], \
                                                   solid_ycoords[:,surfU_vib_indices], \
        ...
        vx_ch = np.zeros([self.chunksize, self.Nx, self.Ny, self.Nz], dtype=np.float32)
        ...
        if solid_start != None:
            ...
            for i in range(self.Nx):
                ...
                # Fluid -----------------------------------------
                        ...
                        Nzero_fluid = np.less(N_fluid_mask[:, i, j, k], 1)
                        N_fluid_mask[Nzero_fluid, i, j, k] = 1
                ...
                # Bulk -----------------------------------
                        ...
                        Nzero_bulk = np.less(N_bulk_mask[:, i, j, k], 1)
                        N_bulk_mask[Nzero_bulk, i, j, k] = 1
                ...
                # -----------------------------------------------------
                        ...
                        # Density (whole fluid) ----------------------------
                        den_ch[:, i, j, k] = (N_fluid_mask[:, i, j, k] / self.A_per_molecule) / vol_fluid[i, j, k]
                        # Density (bulk) -----------------------------------
                        den_bulk_ch[:, i, j, k] = (N_bulk_mask[:, i, j, k] / self.A_per_molecule) / vol_bulk_cell[i, j, k]

                        # Mass flux (whole fluid) ----------------------------
                        jx_ch[:, i, j, k] = vx_ch_whole[:, i, j, k] * den_ch[:, i, j, k]

                        # Mass flow rate (whole fluid) -----------------------
                        mflowrate_ch[:, i, j, k] = (N_fluid_mask[:, i, j, k] / self.A_per_molecule) * vx_ch_whole[:, i, j, k]

                        # Temperature ----------------------------
                        # COM velocity in the bin
                        uCOMx = np.sum(fluid_vx_t * mask_fluid, axis=1) / (N_fluid_mask[:, i, j, k])
                        uCOMy = np.sum(fluid_vy_t * mask_fluid, axis=1) / (N_fluid_mask[:, i, j, k])
                        uCOMz = np.sum(fluid_vz_t * mask_fluid, axis=1) / (N_fluid_mask[:, i, j, k])

                        # In the upper surface
                        uCOMx_surfU = np.sum(surfU_vx_t * maskxU_vib, axis=1) / (N_Upper_vib_mask[:, i, j, k])
                        uCOMy_surfU = np.sum(surfU_vy_t * maskxU_vib, axis=1) / (N_Upper_vib_mask[:, i, j, k])
                        uCOMz_surfU = np.sum(surfU_vz_t * maskxU_vib, axis=1) / (N_Upper_vib_mask[:, i, j, k])
                        ...
                        temp_ch[:, i, j, k] = ((self.mf * sci.gram / sci.N_A) * \
                                            np.sum(peculiar_v**2 , axis=1) * \
                                            A_per_fs_to_m_per_s**2)  / \
                                            (3 * N_fluid_mask[:, i, j, k] * sci.k /self.A_per_molecule)  # Kelvin

                        temp_ch_solid[:, i, j, k] = ((self.ms * sci.gram / sci.N_A) * \
                                            np.sum(peculiar_v_solid**2 , axis=1) * \
                                            A_per_fs_to_m_per_s**2)  / \
                                            (3 * N_Upper_vib_mask[:, i, j, k] * sci.k)  # Kelvin


                        # Virial pressure--------------------------------------
                        # Simulations with virial calculation
                        try:
                            Wxx_ch[:, i, j, k] = np.sum(virial[:, fluid_idx, 0] * mask_bulk, axis=1) / vol_bulk_cell[i,0,k]
                            Wyy_ch[:, i, j, k] = np.sum(virial[:, fluid_idx, 1] * mask_bulk, axis=1) / vol_bulk_cell[i,0,k]
                            Wzz_ch[:, i, j, k] = np.sum(virial[:, fluid_idx, 2] * mask_bulk, axis=1) / vol_bulk_cell[i,0,k]
                        ...
                        try:
                            Wxy_ch[:, i, j, k] = np.sum(virial[:, fluid_idx, 3] * mask_fluid, axis=1) / vol_fluid[i,0,k]
                            Wxz_ch[:, i, j, k] = np.sum(virial[:, fluid_idx, 4] * mask_fluid, axis=1) / vol_fluid[i,0,k]
                            Wyz_ch[:, i, j, k] = np.sum(virial[:, fluid_idx, 5] * mask_fluid, axis=1) / vol_fluid[i,0,k]
