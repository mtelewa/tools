...
class derive_data:
    ...
    def __init__(self, skip, infile_x, infile_z, mf, pumpsize):
        ...
        pumpsize: float
            ...
            It is used to Computes the pressure gradient.
        """
    ...
    def velocity(self):
        ...
        try:
            vx_full_x_whole = np.array(self.data_x.variables["Vx_whole"])[self.skip:] * A_per_fs_to_m_per_s  # m/s
        except KeyError:
            vx_full_x_whole = 0
    ...
    def mflux(self):
        ...
        mflowrate_full_x = np.array(self.data_x.variables["mdot"])[self.skip:] * A_per_fs_to_m_per_s
    ...
    def heat_flux(self):
        """
        Computes the heat flux according to Irving-Kirkwood expression
        """
    ...
    def viscosity_gk(self):
        """
        Computes dynamic viscosity from equilibrium MD
    ...
    def lambda_gk(self):
        """
        Computes thermal conductivity (lambda) from equilibrium MD
    ...
    def lambda_ecouple(self, thermo_out):
        """
        Computes thermal conductivity (lambda) from Fourier's law
    ...
    def slip_length(self):
        """
        Computes the slip length

        Returns
        -------
        Ls: int, slip length in nm (obtained from extrapolation of the height at velocty of zero)
        Vs : int, slip velocity in m/s (Slip velocty * Shear rate)
        xdata: arr, Positions to inter/extrapolates= '-', mark
        extrapolate: arr, Extrapolated data
        """
        dd = derive_data(self.skip, self.infile_x, self.infile_z, self.mf, self.pumpsize)
        vels = dd.velocity()['vx_Z']
    ...
    def coexistence_densities(self):
        ...
        Returns:
        --------
        rho_l and rho_v: floats, Liquid and vapor densities averaged over time
        """
        dd = derive_data(self.skip, self.infile_x, self.infile_z, self.mf, self.pumpsize)
        ...
        rho_vapour = np.min(dd.density()['den_Z'][10:-10])
    ...
    def surface_tension(self):
        """
        Computes the surface tension (γ in N/m) from an equilibrium Liquid/Vapor interface
    ...
    def sf(self):
        """
        Computes the structure factor for the liquid and solid
        ...
        # skip here is used to truncate the computed SF
