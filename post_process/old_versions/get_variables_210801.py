...
class derive_data:
    ...
    def velocity(self):
        ...
        vx = np.array(self.data.variables["Vx"])[self.skip:]
    ...
    def vx_distrib(self):
        ...
        fluid_vx = np.array(self.data.variables["Fluid_Vx"])
        # fluid_vx = np.reshape(fluid_vx,(steps, 14700))[self.skip:] / (len(self.time))
    ...
    def density(self):
        ...
        # Bulk Density ---------------------
        density_Bulk = np.array(self.data.variables["Density_Bulk"])
    ...
    def sigwall(self):
        ...
        sigzz_chunkX = np.mean(fz_Upper,axis=0) * pa_to_Mpa / chunk_A
        print(sigzz_chunkX)
