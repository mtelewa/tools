...
import sys
import os
from get_variables_210830 import derive_data as dd
import funcs
...
class plot_from_ds:
    ...
    def __init__(self):
        ...
        #     self.Nz_mod = len(dd(datasets_x[i], datasets_z[i], skip).velocity()['height_array_mod'])
    ...
    def vx_height(self, label=None, err=None, lt='-', mark='o', opacity=1.0):
        ...
        for i in range(len(datasets_z)):
            ...
            label=input('Label:')
            self.ax.plot(height_array_mod[i, :], vx_chunkZ_mod[i, :],
