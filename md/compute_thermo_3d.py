...
class ExtractFromTraj:
    ...
    def velocity(self):
        ...
        vx_full_whole = np.array(self.data.variables["Vx_whole"])[self.skip:] * Angstromperfs_to_mpers  # m/s
        ...
        # vx_chunkX = np.mean(vx_full_whole, axis=(0,2,3))
        # vx_t = np.mean(vx_full_whole, axis=(1,2))

        data_xy = np.mean(vx_full_whole, axis=(0,3))
        data_xt = np.mean(vx_full_whole, axis=(2,3))


        # Discard timesteps with zero average velocity in any bin
        # vx_full_z = ma.masked_where(vx_full_z == 0, vx_full_z)

        # vx_chunkZ = np.mean(vx_full_z, axis=(0,1,2))
        # print(vx_chunkZ)

        return {'data_xy':data_xy, 'data_xt':data_xt}
